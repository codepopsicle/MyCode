...
class LoadNotification(Resource):
      ...
      def post(self):
          ...
          try:
              ...
              query2= "SELECT TranID, Amt FROM customertrannotification WHERE isViewed = 1 AND CustID = %s AND TransactionVerified = 1"
              param = (data,)
              ...
              ret=[]
              for x in data1:
                  sql="SELECT CompanyName,Locality,TranCity,TransactionDate FROM transactionmasterlist WHERE TransactionID=%s AND TransactionVerified = 1"
                  params=(x[0],)
