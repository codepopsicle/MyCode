...
class PostReview(Resource):
	def post(self):
		...
		try:
			...
			_userStarRating = int(args['StarRating'])
			...
			if len(data):
				...
				query = """SELECT Locality 
							FROM outletlist 
							WHERE outletid = (SELECT outletid FROM transactionlist 
				...
				# if Locality exists
				if len(data):
					_userLocality = data[0][0]
					conn = mysql.connect()
					cursor = conn.cursor()
					# pdb.set_trace()
					query = """INSERT INTO prodreviewlist
								(CustomerID, Barcode, Review, StarRating, TransactionID, CustomerName, Locality)
								VALUES (%s, %s, %s, %s, %s, %s, %s)
							"""
					params = (CustomerID, _userBarcode,_userReview, _userStarRating, _userTransactionID, FullName, _userLocality)
					cursor.execute(query,params)
					data = cursor.fetchall() 
					cursor.close()
					conn.commit()
