...
class VerifyOtp(Resource):
	def post(self):
		...
		try:
			...
			conn = mysql.connect()
			cursor = conn.cursor() 
			cursor.callproc('whizzbuytest.spGetOtp',(_userMobile,))
			...
			if len(data):
				...
				if otp == _userOtp:
					conn = mysql.connect()
					cursor = conn.cursor() 
					cursor.callproc('whizzbuytest.spCheckVerified',(_userMobile,))
					...
					if len(data):
						conn = mysql.connect()
						cursor = conn.cursor() 
						cursor.callproc('whizzbuytest.spSetIsVerified',(_userMobile,))
					...
					cursor.callproc('whizzbuytest.spGetOtpCounter',(_userMobile,))
					# pdb.set_trace()
					data = cursor.fetchall() 
					cursor.close()
					...
					if int(data[0][0]) < 3:
						conn = mysql.connect()
						cursor = conn.cursor() 
						cursor.callproc('whizzbuytest.spIncrOtpCounter',(_userMobile,int(data[0][0]) +1))
						...
						conn = mysql.connect()
						cursor = conn.cursor() 
						cursor.callproc('whizzbuytest.spUpdateOtp',(_userMobile, otp, c_time))
						cursor.close()
						conn.commit()
