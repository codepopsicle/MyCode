...
class Login(Resource):
	def post(self):
		try:
			...
			query1 = '''SELECT LifeStyle FROM customerlist WHERE CustomerID = (SELECT CustomerID FROM customermaster WHERE MobileNO = %s)'''
			params1 = (_userMobile,)
			...
			if len(data):
				isVerified = data[0][0]
				isLoggedIn = data[0][1]
				# Check whether the user is not logged in already and is verifieded
				if isLoggedIn == 1 and isVerified == 1:
					...
					if isLoggedIn == 2:
						...
						return {'Status':'1001', 'Message': 'Account is already logged in', 'newuser': newuser}
